...
const UnifiedRouter = () => {
	...
	const serverIdFromPath = isServerRoute
	...
	const serverInternalId = ServerContext.useStoreState(
		...
		nests
			.find((nest) =>
				nest.attributes.relationships.eggs.data.find(
					(egg) => egg.attributes.uuid === egg_id,
				),
			)
			...
			<HeaderProvider>
				<Fragment key={"unified-router"}>
					<div className="flex flex-col w-full h-full relative">
						<AppHeader
							serverId={
								isServerRoute ? serverInternalId?.toString() : undefined
							}
						/>
