...
export default async (
	...
	if (daemonType?.toLowerCase() === "wings") {
		...
		try {
			...
			if (docker_image) {
				await http.put(
					`/api/client/servers/${daemonType}/${uuid}/settings/docker-image`,
					{
						docker_image,
					},
				);
			...
			if (startup_command) {
				console.warn(
					"Custom startup command update not supported for Wings daemon - using egg default",
				);
			}
			...
			if (should_wipe) {
				...
				if (files.length > 0) {
					...
					await http.post(
						`/api/client/servers/${daemonType}/${uuid}/files/delete`,
						{
							root: "/",
							files: fileNames,
						},
			...
			await http.post(
				`/api/client/servers/${daemonType}/${uuid}/settings/reinstall`,
			);
		} catch (error) {
