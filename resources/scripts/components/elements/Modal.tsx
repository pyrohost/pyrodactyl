			...
			<AnimatePresence>
				{visible && (
					<DialogContext.Provider
						value={{ setIcon, setFooter, setIconPosition }}
					>
						<HDialog
							static
							as={motion.div}
								...
								>
									...
									>
										<div className="place-content-between flex items-center m-6">
											{title && (
												<h2 className={`text-2xl text-zinc-100`}>{title}</h2>
											)}
											{dismissable && (
												<button
													onClick={onDismissed}
													className={
														"opacity-45 hover:opacity-100 p-6 -m-6 cursor-pointer"
													}
												>
										...
										<div className={"flex px-6 overflow-y-auto"}>
											...
											<div className={"flex-1 max-h-[70vh] min-w-0"}>
												...
												{closeButton && (
													<div
														className={`my-6 sm:flex items-center justify-end`}
													>
														<ActionButton
															onClick={onDismissed}
