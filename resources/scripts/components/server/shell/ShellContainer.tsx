			...
			switch (ruleName) {
				...
				case "min": {
					if (ruleValue && value) {
						...
						if (value.length < minValue) {
							errors.push(
								`${variable.name} must be at least ${minValue} characters.`,
							);
						}
					}
					break;
				...
				case "max": {
					if (ruleValue && value) {
						...
						if (value.length > maxValue) {
							errors.push(
								`${variable.name} may not be greater than ${maxValue} characters.`,
							);
						}
					}
					break;
				...
				case "between": {
					if (ruleValue && value) {
						...
						if (value.length < min || value.length > max) {
							errors.push(
								`${variable.name} must be between ${min} and ${max} characters.`,
							);
						}
					}
					break;
				...
				case "alpha_num":
					if (value && !/^[a-zA-Z0-9]+$/.test(value)) {
						errors.push(
							`${variable.name} may only contain letters and numbers.`,
						);
					}
					break;
				...
				case "alpha_dash":
					if (value && !/^[a-zA-Z0-9_-]+$/.test(value)) {
						errors.push(
							`${variable.name} may only contain letters, numbers, dashes and underscores.`,
						);
					}
					break;
				...
				default:
					...
					if (
						...
						console.warn(
							`Unknown validation rule: ${ruleName} for variable ${variable.name}`,
						);
					}
					break;
			}
...
const SoftwareContainer = () => {
	...
	const currentEggName = useMemo(() => {
		...
		const foundNest = nests.find((nest) =>
			nest?.attributes?.relationships?.eggs?.data?.find(
	...
	useEffect(() => {
		if (backups) {
			...
			setShouldBackup(
				backupLimit !== 0 &&
					(backupLimit === null || backups.backupCount < backupLimit),
			);
		}
	}, [backups, backupLimit]);
	...
	const handleEggSelection = async (egg: Egg) => {
		...
		try {
			const preview = await previewEggChange(
				uuid,
				egg.attributes.id,
			...
			if (preview.warnings && preview.warnings.length > 0) {
				const subdomainWarning = preview.warnings.find(
					(w) => w.type === "subdomain_incompatible",
				);
				if (subdomainWarning) {
			...
			preview.variables.forEach((variable) => {
				const existingVar = data?.variables.find(
					(v) => v.envVariable === variable.env_variable,
				);
				initialVariables[variable.env_variable] =
					existingVar?.serverValue || variable.default_value || "";
			});
			...
			if (
				preview.default_docker_image &&
				availableDisplayNames.includes(preview.default_docker_image)
			) {
	...
	const handleVariableChange = (envVariable: string, value: string) => {
		...
		if (eggPreview) {
			const variable = eggPreview.variables.find(
				(v) => v.env_variable === envVariable,
			);
			if (variable) {
	...
	const handleOperationComplete = (operation: ServerOperation) => {
		if (operation.is_completed) {
			toast.success(
				"Your software configuration has been applied successfully",
			);
			...
			toast.error(
				operation.message || "The software configuration change failed",
			);
		}
	};
	...
	const handleOperationError = (error: Error) => {
		toast.error(
			error.message || "An error occurred while monitoring the operation",
		);
	};
						...
						<div className="space-y-4">
							...
							<div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
								...
								<div>
									...
									{eggPreview.docker_images &&
									Object.keys(eggPreview.docker_images).length > 1 ? (
										<DropdownMenu>
										...
										<div className="w-full px-3 py-2 bg-[#ffffff08] border border-[#ffffff12] rounded-lg text-sm text-neutral-200">
											{(eggPreview.docker_images &&
												Object.keys(eggPreview.docker_images)[0]) ||
												"Default Image"}
										</div>
									)}
									<p className="text-xs text-neutral-400 mt-1">
											...
											{variable.user_editable ? (
												<div>
													<input
														...
														placeholder={
															variable.default_value || "Enter value..."
														}
								...
								<div className="flex items-center justify-between p-4 bg-[#ffffff08] border border-[#ffffff12] rounded-lg hover:border-[#ffffff20] transition-colors">
									<div className="flex-1 min-w-0 pr-4">
										...
										<p className="text-xs text-neutral-400 leading-relaxed">
											{backupLimit !== 0 &&
											(backupLimit === null ||
												(backups?.backupCount || 0) < backupLimit)
						...
						<div className="p-4 bg-[#ffffff08] border border-[#ffffff12] rounded-lg">
							...
							<div className="space-y-2">
								<div className="flex justify-between items-center py-2 px-3 bg-[#ffffff08] rounded-lg">
									...
									<span
										className={
											shouldBackup ? "text-green-400" : "text-neutral-400"
										}
									>
										{shouldBackup ? "Yes" : "No"}
									</span>
								...
								<div className="flex justify-between items-center py-2 px-3 bg-[#ffffff08] rounded-lg">
									...
									<span
										className={
											shouldWipe ? "text-amber-400" : "text-neutral-400"
										}
									>
										{shouldWipe ? "Yes" : "No"}
									</span>
							...
							<div className="flex items-start gap-3">
								...
								<div>
									...
									<span className="text-sm font-bold mt-4">
										Please ensure you have backups of important data before
										proceeding.
									</span>
								</div>
							</div>
	...
	return (
		<ServerContentBlock title="Software Management">
			<div className="space-y-6">
				<MainPageHeader direction="column" title="Software Management">
					<p className="text-neutral-400 leading-relaxed">
						Change your server&apos;s game or software with our guided
			...
			<ConfirmationModal
				...
				buttonText={
					wipeCountdown > 0
						? `Yes, Wipe Files (${wipeCountdown}s)`
						: "Yes, Wipe Files"
				}
					...
					<div className="flex items-start gap-3 p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
						...
						<div>
							<h4 className="text-red-400 font-semibold mb-2">
								DANGER: No Backup Selected
							</h4>
							<p className="text-sm text-neutral-300">
								You have chosen to wipe all files{" "}
								<strong>without creating a backup</strong>. This action will{" "}
								<strong>permanently delete ALL files</strong> on your server and
								cannot be undone.
							</p>
						</div>
					</div>
