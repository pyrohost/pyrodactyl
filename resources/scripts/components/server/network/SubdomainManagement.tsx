...
// or wherever your i18n instance is
import {
    SubdomainInfo,
...
const validationSchema = yup.object().shape({
    subdomain: yup
        .string()
        .required(i18n.t('server.network.subdomain.required'))
        .min(1, i18n.t('server.network.subdomain.min_name'))
        .max(63, i18n.t('server.network.subdomain.max_name'))
        .matches(/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/i, i18n.t('server.network.subdomain.invalid')),
    domain_id: yup.string().required(i18n.t('server.network.subdomain.domain_required')),
});
...
const SubdomainManagement = () => {
        ...
        async (subdomain: string, domainId: string) => {
            ...
            try {
                ...
                setAvailabilityStatus({
                    checked: true,
                    available: false,
                    message: t('server.network.subdomain.failed_ability_check'),
                });
            } finally {
    ...
    const handleDeleteSubdomain = async () => {
        if (!confirm(t('server.network.subdomain.final_delete_check'))) {
            return;
        }
                ...
                <div className='flex items-center justify-center py-12'>
                    <div className='flex flex-col items-center gap-3'>
                        ...
                        <p className='text-sm text-neutral-400'>
                            {t('server.network.subdomain.load_subdomain_config')}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className='bg-gradient-to-b from-[#ffffff08] to-[#ffffff05] border-[1px] border-[#ffffff12] rounded-xl p-6 shadow-sm'>
                <div className='flex items-center justify-between mb-6'>
                    <h3 className='text-xl font-extrabold tracking-tight'>
                        {t('server.network.subdomain.subdomain_management')}
                    </h3>
                </div>
                <div className='flex flex-col items-center justify-center py-12'>
                    <div className='text-center'>
                        ...
                        <h4 className='text-md font-medium text-zinc-200 mb-1'>
                            {t('server.network.subdomain.no_domains_configured')}
                        </h4>
                        <p className='text-sm text-zinc-400 max-w-sm'>{t('server.network.subdomain.contact_admin')}</p>
                    </div>
                </div>
            </div>
        );
                ...
                <div className='space-y-4'>
                        ...
                        <div className='flex items-center justify-between'>
                            <div>
                                <p className='text-sm text-zinc-400 mb-2'>
                                    {t('server.network.subdomain.current_subdomain')}
                                </p>
                    ...
                    <div className='flex items-center justify-end gap-3 pt-4 border-t border-[#ffffff15]'>
                        <ActionButton
                            ...
                            size='sm'
                        >
                            {loading ? t('common.deleting') : t('server.network.subdomain.delete_subdomain')}
                        </ActionButton>
                        <ActionButton
                            type='button'
                            variant='primary'
                                    ...
                                    >
                                        {checkingAvailability ? (
                                            <div className='flex items-center text-sm text-blue-300'>
                                                ...
                                                {t('server.network.subdomain.check_availability')}
                                            </div>
                                        ) : (
                                            availabilityStatus && (
                                                <div
