        ...
        <Formik onSubmit={submit} initialValues={{ name: files.length > 1 ? '' : files[0] || '' }}>
            {({ isSubmitting, values }) => (
                <Modal
                    ...
                    title={useMoveTerminology ? 'Moviendo archivos/carpetas' : 'Renombrando archivos/carpetas'}
                >
                    <Form className={`w-full`}>
                        <div className='w-full'>
                            <Field type={'string'} id={'file_name'} name={'name'} label={'Nombre del archivo'} autoFocus />
                            {useMoveTerminology && (
                            ...
                            <div className={`flex justify-end w-full my-6`}>
                                <ActionButton variant='primary' type='submit'>
                                    {useMoveTerminology ? 'Mover' : 'Renombrar'}
                                </ActionButton>
                            </div>
                        </div>
                    </Form>
