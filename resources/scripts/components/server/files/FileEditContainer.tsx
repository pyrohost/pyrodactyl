...
const FileEditContainer = () => {
	...
	return (
		<PageContentBlock
			title={action === "edit" ? `Editing ${filename}` : `New File`}
		...
		>
			...
			{[".pyroignore", ".pyroignore"].includes(filename) ? (
				<div
					className={`mb-4 p-4 border-l-4 bg-neutral-900 rounded-sm border-cyan-400`}
				>
					<p className={`text-neutral-300 text-sm`}>
						You&apos;re editing a{" "}
						<code className={`font-mono bg-black rounded-sm py-px px-1`}>
							.pyroignore
						</code>{" "}
			...
			<div className="flex flex-row items-center gap-4 absolute top-2.5 right-2">
				<DropdownMenu>
					...
					<DropdownMenuContent
						className="max-h-50 overflow-auto z-99999"
						sideOffset={8}
					>
						<For
							each={languages.sort((a, b) => a.name.localeCompare(b.name))}
							memo
						>
							{(language) => (
								<DropdownMenuItem
