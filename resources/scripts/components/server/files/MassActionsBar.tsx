...
const MassActionsBar = () => {
	...
	return (
		<>
			<div
				className={`pointer-events-none fixed bottom-0 z-20 left-0 right-0 flex justify-center`}
			>
				...
				>
					...
					>
						<div
							className={`flex items-center space-x-4 pointer-events-auto rounded-sm p-4 bg-black/50`}
						>
							<ActionButton
								onClick={() => setShowMove(true)}
								disabled={loading}
							>
								{loading && loadingMessage.includes("Moving") && (
									<Spinner size="small" />
								)}
								Move
							</ActionButton>
							<ActionButton onClick={onClickCompress} disabled={loading}>
								{loading && loadingMessage.includes("Archiving") && (
									<Spinner size="small" />
								)}
								Archive
							</ActionButton>
							<ActionButton
								...
								disabled={loading}
							>
								{loading && loadingMessage.includes("Deleting") && (
									<Spinner size="small" />
								)}
								Delete
							</ActionButton>
