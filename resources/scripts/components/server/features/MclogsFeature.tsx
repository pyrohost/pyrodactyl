...
const useLogAnalysis = () => {
	...
	useWebsocketEvent(SocketEvent.CONSOLE_OUTPUT, (data: string) => {
		...
		if (consoleBufferRef.current.length > MAX_CONSOLE_BUFFER) {
			...
			consoleBufferRef.current = consoleBufferRef.current.slice(
				-MAX_CONSOLE_BUFFER,
			);
		}
	});
			...
			try {
				...
				if (showToast) {
					if (result.analysis?.problems?.length > 0) {
						toast.success(
							`Analysis complete - ${result.analysis.problems.length} issue(s) found`,
						);
					} else {
				...
				const errorMessage =
					err instanceof Error ? err.message : "Failed to analyze server logs";
	...
	useEffect(() => {
		...
		if (previousStatusRef.current !== "offline" && status === "offline") {
			const hasCrashIndicators = consoleBufferRef.current.some((line) =>
				isCrashLine(line),
			);
...
export const CrashAnalysisCard = () => {
	const { analyzing, analysis, error, showCard, dismissCard } =
		useLogAnalysis();
	...
	const renderLoadingState = () => (
		...
		>
			...
			<p className="text-neutral-400 mt-2 text-center max-w-md">
				We&apos;re analyzing your server logs with mclo.gs to identify potential
					...
					<div className="flex-1">
						...
						{(/latest\.log/i.test(error!) ||
							...
							<p className="text-neutral-400 mt-3 text-sm">
								This usually means the log file doesn&apos;t exist yet. Try
								starting your server to generate logs first.
							</p>
						)}
					</div>
				</div>
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-neutral-300 mt-2">
								No specific issues were found in your server logs. The crash may
								be due to configuration problems or resource limitations.
							</p>
						</div>
					</div>
				</div>
