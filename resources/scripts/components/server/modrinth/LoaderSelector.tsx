...
export const LoaderSelector = ({
	maxVisible = 7,
	featuredLoaders = DEFAULT_LOADERS,
}: LoaderSelectorProps) => {
	const { loaders, selectedLoaders, setSelectedLoaders } =
	...
	const { featured, other, filtered } = useMemo(() => {
		...
		const filteredLoaders = searchQuery
			? loaders.filter(
					(loader) =>
						fuzzyMatch(loader.name, searchQuery) ||
						fuzzyMatch(loader.id, searchQuery),
				)
			: loaders;
	...
	const hasMoreLoaders =
		featured.length + other.length > maxVisible && !searchQuery;
