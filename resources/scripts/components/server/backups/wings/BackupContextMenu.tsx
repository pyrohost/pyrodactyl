...
const BackupContextMenu = ({ backup }: Props) => {
	...
	const doDeletion = async () => {
		...
		try {
			await http.delete(
				`/api/client/servers/${daemonType}/${uuid}/backups/${backup.uuid}`,
				{
					data: {
	...
	const doRestorationAction = async () => {
		...
		try {
			await http.post(
				`/api/client/servers/${daemonType}/backups/${backup.uuid}/restore`,
				{
				...
				<div className="space-y-4">
					<div className="space-y-2">
						...
						<p className="text-sm text-zinc-400">
							Your server will be stopped during the restoration process. You
							will not be able to control the power state, access the file
						...
						<div className="flex items-start space-x-3">
							...
							<div className="space-y-1">
								...
								<p className="text-xs text-red-300">
									All current files and server configuration will be deleted and
						...
						{hasTwoFactor && (
							<div>
								...
								<input
									...
									onChange={(e) =>
										setRestoreTotpCode(e.target.value.replace(/[^0-9]/g, ""))
									}
									disabled={loading}
								/>
							</div>
						...
						{hasTwoFactor && (
							<div>
								...
								<input
									...
									onChange={(e) =>
										setDeleteTotpCode(e.target.value.replace(/[^0-9]/g, ""))
									}
									disabled={loading}
								/>
							</div>
