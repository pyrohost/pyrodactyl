...
const BackupContainer = () => {
	...
	const hasTwoFactor = useStoreState(
		(state: ApplicationStore) => state.user.data?.useTotp || false,
	);
	...
	const backupStorageLimit = ServerContext.useStoreState(
		(state) => state.server.data!.featureLimits.backupStorageMb,
	);

	// Check if any backup operation is in progress
	const hasActiveOperation = Object.values(liveProgress).some(
		(op) => !op.completed,
	);
	...
	const submitBackup = async (
		values: BackupValues,
		{ setSubmitting }: FormikHelpers<BackupValues>,
	) => {
		clearFlashes("backups:create");
	...
	const handleDeleteAll = async () => {
		...
		try {
			...
			toast.success(
				"All backups and repositories are being deleted. This may take a few minutes.",
			);
			...
			<ServerContentBlock title={"Backups"}>
				...
				<MainPageHeader direction="column" title={"Backups"}>
					<p className="text-sm text-neutral-400 leading-relaxed">
						Create and manage server backups to protect your data. Schedule
						automated backups, download existing ones, and restore when needed.
					</p>
				</MainPageHeader>
		...
		<ServerContentBlock title={"Backups"}>
					...
					<Can action={"backup.create"}>
						<div className="flex flex-col sm:flex-row items-center justify-end gap-4">
							<div className="flex flex-col gap-1 text-center sm:text-right">
								...
								{backupLimit === null && (
									<p className="text-sm text-zinc-300">{backupCount} backups</p>
								)}
								{backupLimit > 0 && (
								...
								{backupLimit === 0 && (
									<p className="text-sm text-red-400">Backups disabled</p>
								)}
			...
			>
				<p className="text-sm text-neutral-400 leading-relaxed">
					Create and manage server backups to protect your data. Schedule
					automated backups, download existing ones, and restore when needed.
					Backups are deduplicated, meaning unchanged files are only stored once
					across all backups
				</p>
			</MainPageHeader>
							...
							<div className="flex items-start gap-3">
								...
								<div className="text-sm">
									...
									<p className="text-red-400 mt-1">
										The selected backup files and their snapshots will be
										permanently deleted. You will not be able to restore them.
									</p>
								</div>
							</div>
						</div>
						...
						{backups.map((backup) =>
							daemonType === "elytra" ? (
								<BackupItemElytra
									...
									isSelectable={selectableBackups.some(
										(b) => b.uuid === backup.uuid,
									)}
...
const BackupContainerWrapper = () => {
	...
	const handleBackupStatus = useCallback(
		(rawData: any) => {
			...
			const last_updated_at = timestamp
				? new Date(timestamp * 1000).toISOString()
				: new Date().toISOString();
			const isDeletionOperation =
				operation === "delete" || data.deleted === true;
			...
			if (status === "completed" && progress === 100) {
				if (isDeletionOperation) {
					...
					const checkForBackup = async (attempts = 0) => {
						...
						const backupExists = currentBackups?.items?.some(
							(b) => b.uuid === backup_uuid,
						);
