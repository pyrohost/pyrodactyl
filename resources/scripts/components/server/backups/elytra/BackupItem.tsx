...
const BackupItem = ({
	...
	const handleRetry = async () => {
		...
		try {
			...
			addFlash({
				...
				key: "backup",
				message:
					error instanceof Error ? error.message : "Failed to retry backup.",
			});
		}
	};
	...
	const showProgressBar =
		isActive || (backup.status === "completed" && backup.isLiveOnly);
			...
			<div className="flex items-center gap-3 w-full">
				...
				<div className="flex-1 min-w-0">
					...
					{backup.status === "failed" && backup.message && (
						<p className="text-xs text-red-400 truncate mb-1.5">
							{backup.message}
						</p>
					)}
