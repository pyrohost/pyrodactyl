...
export const useUnifiedBackups = () => {
	...
	const createBackup = useCallback(
		async (name: string, ignored: string, isLocked: boolean) => {
			const { default: createServerBackup } = await import(
				"@/api/server/backups/createServerBackup"
			);
	...
	const retryBackup = useCallback(
		async (backupUuid: string) => {
			const { retryBackup: retryBackupApi } = await import(
				"@/api/server/backups"
			);
	...
	const renameBackup = useCallback(
		async (backupUuid: string, newName: string) => {
			...
			await http.post(
				`/api/client/servers/${daemonType}/${uuid}/backups/${backupUuid}/rename`,
				{ name: newName },
			);
			mutate();
		},
	...
	const toggleBackupLock = useCallback(
		async (backupUuid: string) => {
			const http = (await import("@/api/http")).default;
			await http.post(
				`/api/client/servers/${daemonType}/${uuid}/backups/${backupUuid}/lock`,
			);
			mutate();
		},
		[uuid, mutate],
