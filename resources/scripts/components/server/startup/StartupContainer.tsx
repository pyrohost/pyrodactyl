		...
		revertDockerImage(uuid)
			.then(() => {
				...
				const defaultImage = data
					? Object.values(data.dockerImages)[0] || ""
					: "";
		...
		>
			...
			>
				<div className="space-y-3">
					<p>
						This will revert your server&apos;s Docker image back to the
						egg&apos;s default specification.
					</p>
					<div className="bg-linear-to-b from-amber-500/10 to-amber-600/5 border border-amber-500/20 rounded-xl p-3">
						<p className="text-sm text-amber-200">
							<span className="font-medium">⚠️ Warning:</span> You will not be
							able to set a custom image back without contacting support.
						</p>
					</div>
					<p className="text-sm text-neutral-400">
			...
			<div className="space-y-6">
				<MainPageHeader direction="column" title="Startup Settings">
					<p className="text-sm text-neutral-400 leading-relaxed">
						Configure how your server starts up. These settings control the
				...
				<div className="space-y-6">
					<TitledGreyBox title={"Startup Command"} className="p-6">
						<div className="space-y-4 mb-6">
							<p className="text-sm text-neutral-400 leading-relaxed">
								Configure the command that starts your server. You can edit the
								raw command or view the processed version with variables
								resolved.
							</p>
						</div>
						{editingCommand ? (
					...
					<TitledGreyBox title={"Docker Image"} className="p-6">
						<div className="space-y-4 mb-6">
							<p className="text-sm text-neutral-400 leading-relaxed">
								The container image used to run your server. Different images
									...
									<DropdownMenu>
											...
											<button className="w-full flex items-center justify-between gap-3 font-medium text-sm sm:text-base px-3 py-3 sm:px-4 sm:py-3 rounded-md bg-linear-to-b from-[#ffffff10] to-[#ffffff09] border border-[#ffffff15] hover:from-[#ffffff15] hover:to-[#ffffff10] hover:border-[#ffffff25] transition-all cursor-pointer touch-manipulation">
												<span className="truncate text-left font-mono text-neutral-200">
													{Object.keys(data.dockerImages).find(
														(key) =>
															data.dockerImages[key] === variables.dockerImage,
										...
										>
											<DropdownMenuRadioGroup
												...
												onValueChange={(value) =>
													updateSelectedDockerImage(value)
												}
											>
									...
									<div className="bg-linear-to-b from-amber-500/10 to-amber-600/5 border border-amber-500/20 rounded-xl p-3 sm:p-4">
										<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
											<div className="flex-1">
												<p className="text-sm text-amber-200">
													<span className="font-medium">Notice:</span> This
													server&apos;s Docker image has been manually set by an
													administrator and cannot be changed through this
													interface.
												</p>
												{canEditDockerImage && (
													<p className="text-xs text-amber-300/80 mt-2">
														You can revert to the egg&apos;s default image, but
														you won&apos;t be able to set it back without
														contacting support.
													</p>
												)}
											</div>
					...
					<div className="space-y-6">
						<div className="space-y-3">
							...
							<p className="text-sm text-neutral-400 leading-relaxed">
								...
								server. These variables can be used to customize server behavior
								and settings.
							</p>
						</div>
