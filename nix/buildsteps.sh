...
mysql -u $mysqluser -p$mysqlrootpass -e "CREATE DATABASE IF NOT EXISTS panel;" \
...
mysql -u $mysqluser -p$mysqlrootpass -e "CREATE USER IF NOT EXISTS 'pyrodactyluser'@'%' IDENTIFIED BY 'pyrodactyl';" \
...
mysql -u $mysqluser -p$mysqlrootpass -e "USE panel; UPDATE users SET root_admin = 1;" # workaround because --admin is broken
